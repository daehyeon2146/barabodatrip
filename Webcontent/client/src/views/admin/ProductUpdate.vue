<template>
   <main class="mt-3">
      <div class="container">
        ...
        <div class="mb-3 row">
          <label  class="col-md-3 col-form-label">언어</label>
          <div class="col-md-9">
            <div class="row">
        ...
        <div class="mb-3 row">
          ...
          <div class="col-md-9">
            <div class="row">
              <div class="col-lg-3 col-md-4 col-sm-2">
                <select class="form-select" v-model.trim="product.tr_tp" @change="changeCategory">
                  <option :value="cate" :key="i" v-for="(cate ,i) in category1" selected>{{cate}}</option>
                </select>
              </div>
              <div class="col-lg-3 col-md-4 col-sm-2">
                <select class="form-select" v-model.trim="product.tr_category" selected>
                  <option :value="cate" :key="i" v-for="(cate ,i) in category2" selected>{{cate}}</option>
                </select>
              </div>
            </div>
          </div>
            ...
            <div class="row">
              <div class="col-lg-3 col-md-4 col-sm-2" :key="i" v-for="(m,i) in productImageList.filter(c=>c.file_type==1)">
                <div class="position-relative">
                  <img :src="`/download/${m.file_path}/${m.file_category}/${this.trSeq}/${m.file_name}`" class="img-fluid" />
                  <div class="position-absolute top-0 end-0" style="cursor:pointer;color:black;">
                    <a @click="deleteImage(m.file_seq,m.file_path,m.file_category,this.trSeq,m.file_name)"><img src="../../img/ico/cancel_btn.png" width="30" height="30"/></a>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="mb-3 row">
          <label class="col-md-3 col-form-label">제품설명이미지 {{product.lan_tp}} {{trTp}}</label>
          <div class="col-md-9">
            <div class="row">
              <div class="col-lg-3 col-md-4 col-sm-2" :key="i" v-for="(m,i) in productImageList.filter(c=>c.file_type==3)">
                <div class="position-relative">
                  <img :src="`/download/${m.file_path}/${m.file_category}/${this.trSeq}/${m.file_name}`" class="img" width="150" height="150"/>
                  <div class="position-absolute top-0 end-0" style="cursor:pointer;color:black;">
                    <a @click="deleteImage(m.file_seq,m.file_path,m.file_category,this.trSeq,m.file_name)"><img src="../../img/ico/cancel_btn.png" width="30" height="30"/></a>
                  </div>
                </div>
        ...
        <div class="mb-3 row">
          <label  class="col-md-2 col-form-label">픽업장소</label>
          <div class="col-md-9">
            <input type="text" class="form-control" v-model.trim="product.tr_place">
          </div>
        </div>
        <div class="mb-3 row">
          <label  class="col-md-2 col-form-label">상품간략설명</label>
          <div class="col-md-9">
        ...
        <div class="mb-3 row">
          <label  class="col-md-2 col-form-label">제품가격</label>
          <div class="col-md-9">
...
export default {
    ...
    methods: {
        async getProduct() {
          let product = await this.$api("/api/productUpdateList",{param:[this.trSeq]});
          if(product.length > 0) {
        ...
        productInsert() {
            ...
            this.$swal.fire({
                ...
                if(result.isConfirmed) {
                    ...
                    // , {param:[{this.product},{tr_seq:this.product.tr_seq]});
                    ,{
                      param : [
                        {
                          tr_title : this.product.tr_title,
                          tr_place : this.product.tr_place,
                          tr_desc : this.product.tr_desc,
                          tr_category : this.product.tr_category,
                          tr_tp : this.product.tr_tp,
                          tr_price : this.product.tr_price,
                          tr_price_tp : this.product.tr_price_tp,
                          tr_days : this.product.tr_days,
                          tr_stat : this.product.tr_stat,
                          max_cnt : this.product.max_cnt,
                          view_yn : this.product.view_yn,
                          lan_tp : this.product.lan_tp
                        },
                        {
                          tr_seq:this.trSeq
                        }
        ...
        async uploadFile(files, type,trSeq) {
          ...
          const { error } = await this.$api(`/upload/${filepath}/${this.product.tr_tp}/${trSeq}/${type}/${name}`, { data });
          if (error) {
